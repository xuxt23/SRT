...
class ReconSingleSlice(luigi.Task, TimeTaskMixin):
    ...
    def output(self):
        filename = f"{dataset_core_path(self.input().path)}_singleslice_cuda.{FORMAT}"
        return luigi.LocalTarget(filename)
    ...
    def run(self):
        ...
        magnification_factor = (SOD + ODD)/SOD
        Vitualdetectorinterval = pixel_size
        # Vitualdetectorinterval = pixel_size/ magnification_factor
        Pictureinterval = pixel_size/ magnification_factor
        angles = np.arange(nangle) * angle_step
        ...
        # Copy result back to host
        RePic = np.ones((nwidth, nwidth, nslice), dtype=np.float32)
        cuda.memcpy_dtoh(RePic.flatten(), volume_gpu)
        # Check if the GPU memory transfer worked
        print(f"After GPU transfer - RePic stats: min={np.min(RePic)}, max={np.max(RePic)}, mean={np.mean(RePic)}")
        
        print("Backprojection completed on GPU")
        # # Add this after reconstruction is complete
        # print("\nChecking reconstruction value ranges...")
        # print(f"Reconstruction range before normalization: min={np.min(Reconstruction)}, max={np.max(Reconstruction)}")
        
        # # Adjust the values if they're all very small or mostly negative
        # if np.max(Reconstruction) - np.min(Reconstruction) < 1e-6:
        #     print("WARNING: Very small range of reconstruction values!")
        
        # if np.max(Reconstruction) <= 0:
        #     print("WARNING: All reconstruction values are negative or zero!")
...
# Add this function to test projection geometry
def test_projection_geometry(SOD,ODD,pixel_size,  nwidth, nslice, nangle, angles):
    """
    Test function to verify that points in the reconstruction volume 
    project correctly onto the detector for visual debugging.
    """
    # Extract geometry parameters

    SDD = SOD + ODD         # Source to Detector Distance

    magnification_factor = SDD / SOD
    detector_interval = pixel_size / magnification_factor
    
    # Define some test points in the reconstruction volume
    # Center, corners, and some points in between
    test_points = [
        (0, 0, 0),                  # Center of volume
        (nwidth/4, nwidth/4, 0),    # Quarter way from center
        (nwidth/2, nwidth/2, 0),    # Edge of volume (if centered at origin)
        (-nwidth/4, nwidth/4, nslice/4)  # Point in negative x, positive y, z
    ]
    
    # Calculate voxel size (assuming we're using virtual detector interval)
    voxel_size = detector_interval
    
    print("===== GEOMETRY DEBUG INFORMATION =====")
    print(f"SOD: {SOD}, ODD: {ODD}, SDD: {SDD}")
    print(f"Detector pixel size: {pixel_size}")
    print(f"Magnification factor: {magnification_factor}")
    print(f"Virtual detector interval: {detector_interval}")
    print(f"Reconstruction voxel size: {voxel_size}")
    print(f"Detector dimensions: {nwidth}x{nslice}")
    print(f"Number of projection angles: {nangle}")
    
    # Test projection for a few angles
    test_angles = [0, 45, 90, 180]  # degrees
    test_angles = [a for a in test_angles if a < nangle * parameters['angle_step']]
    
    for point_idx, (x_idx, y_idx, z_idx) in enumerate(test_points):
        # Convert from indices to physical coordinates
        x = (-x_idx + nwidth/2 + 0.5) * voxel_size
        y = (-y_idx + nwidth/2 + 0.5) * voxel_size
        z = (-z_idx + nslice/2 + 0.5) * voxel_size
        
        print(f"\nTest Point {point_idx+1}: Volume index ({x_idx}, {y_idx}, {z_idx}), Physical coordinates ({x:.2f}, {y:.2f}, {z:.2f})")
        
        for angle_deg in test_angles:
            angle_idx = int(angle_deg / parameters['angle_step'])
            if angle_idx >= nangle:
                continue
            ...
            denom = SOD + x * np.cos(beta) + y * np.sin(beta)
            
            # Check if point is behind source
            if denom <= 0:
                print(f"  Angle {angle_deg}°: Point is behind source!")
                continue
                
            # Calculate detector coordinates
            U_source = SOD * (-x * np.sin(beta) + y * np.cos(beta)) / denom
            V_source = z * SOD / denom
            
            # Calculate detector indices
            u_idx = int(nwidth/2 - U_source/detector_interval)
            v_idx = int(nslice/2 - V_source/detector_interval)
            
            # Check if projection is within detector bounds
            in_bounds = (0 <= u_idx < nwidth and 0 <= v_idx < nslice)
            
            print(f"  Angle {angle_deg}°: Projects to detector at ({u_idx}, {v_idx}) - {'In bounds' if in_bounds else 'OUT OF BOUNDS'}")
            print(f"    Details: U={U_source:.2f}, V={V_source:.2f}, denom={denom:.2f}")
